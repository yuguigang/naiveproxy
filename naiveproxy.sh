...
makesite(){
    ...
    cd /var/www/html
    wget -N https://raw.githubusercontent.com/taffychan/naiveproxy/main/mikutap.zip
...
menu() {
    ...
    echo "#############################################################"
    echo ""
    yellow "注意：部分VPS服务器提供商有可能限制NaiveProxy协议的部署和使用，请谨慎部署"
    echo ""
    ...
    echo " -------------"
    echo -e "  ${GREEN}7.${PLAIN}  修改 NaiveProxy IP优先级"
    echo -e "  ${GREEN}8.${PLAIN}  修改 NaiveProxy 连接端口"
